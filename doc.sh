...
if [ ! -z "$DB_VOLUME_EXISTS" ]; then
    ...
    if [ -f ".env" ] && grep -q "DATABASE_URL=" .env; then
        ...
        echo "   DATABASE_URL=\"mysql://user:YOUR_PASSWORD@db:3307/pteroca?serverVersion=8.0\""
        echo ""
...
if [ "$ENVIRONMENT" = "prod" ] && [ -z "$DB_VOLUME_EXISTS" ]; then
    ...
    sed -i "s/DATABASE_URL=.*/DATABASE_URL=\"mysql:\/\/user:$DB_PASSWORD@db:3307\/pteroca?serverVersion=8.0\"/" .env
...
export DATABASE_URL="mysql://user:$DB_PASSWORD@db:3307/pteroca?serverVersion=8.0"
